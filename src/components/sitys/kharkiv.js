...
import style from '../choice/index.css'
...
class Kharkiv extends Component {
  ...
  add = async (event) => { 
      event.preventDefault();
      const api_url = await
      fetch(`http://api.apixu.com/v1/current.json?key=9aab06c4c4024a32be0185051191003&q=харьков`);
         ...
         <div onClick={this.add} className={"weather-result"}>
          <h2>{this.state.data.location.name}</h2>
            <ul className="weather-list">
              ...
              <li className="weather-list-item" title="давление"><img src={barometer} alt="мм" className="weather-icons"></img>{this.state.data.current.pressure_mb +'мм рт. столба'}</li>
            ...
            <p>{"Последнее обновление: "+this.state.data.current.last_updated}</p>
         </div> 
